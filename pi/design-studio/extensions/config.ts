...
export function slugifyTopic(topic: string): string {
  ...
  if (!slug) return "design";

  // Limit to 100 characters to stay well under filesystem limits (typically 255 bytes)
  // and leave room for numeric suffixes like "-2" and the ".md" extension
  const MAX_LENGTH = 100;
  if (slug.length <= MAX_LENGTH) return slug;

  // Truncate at the last word boundary before MAX_LENGTH for cleaner endings
  const truncated = slug.slice(0, MAX_LENGTH);
  const lastDash = truncated.lastIndexOf("-");

  // If we found a dash in a reasonable position (past 60% of max length),
  // break there to avoid cutting mid-word
  if (lastDash > MAX_LENGTH * 0.6) {
    return truncated.slice(0, lastDash);
  }

  // Otherwise hard truncate at MAX_LENGTH
  return truncated;
}
